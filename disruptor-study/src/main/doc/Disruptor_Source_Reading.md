...
4. BatchEventProcessor如何实现批量操作
   ...
   3. '阻塞'假象：在线性拓扑中，比如h1->h2->h3，对于事件流e1、e2、e3、，，，、en，h1每处理完一个事件e1，h2就可以紧接着处理该e1，然后是h3；而不用等到h1处理完这一批en之后，h2才开始处理e1、e2、e3、，，，、en，————这是'完全阻塞'的表现
   4. 解密阻塞假象：因为生产事件是非常快的，所以h1一下子进行了批处理，导致了（e1、e2、e3、，，，、en）同步在一个批处理中完成，而后才设置Sequence，所以h2是一下子看到了这n个事件，然后继续批处理，然后把这一批在一块给h3.
   5. 验证：
